...
public class TimeSlots {
    ...
    public Boolean add(String index, int customerID) { // 整点时间
        ...
        if (slot.getStart().compareTo(openTime) < 0 || slot.getEnd().compareTo(closeTime) > 0) {
            return false;
        }
        if (!checkAvaliable(slot)) {
            return false;
    ...
    //check if the timeslot is able to be added in the timeSlots. true -> can, false -> cannot
    ...
    public String getAviliableSlots() {
